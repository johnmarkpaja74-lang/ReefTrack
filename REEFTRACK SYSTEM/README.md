...
For local demo access without credentials, open `https://localhost:5173/demo`.
This opens the Faculty workspace with a separate demo identity. Use
`/demo?role=technician` or `/demo?role=official` for the other workspaces.
The local `/login` page also enters the demo automatically, without submitting
credentials to the server. Demo entry is available only in development;
production builds retain the normal login flow.
