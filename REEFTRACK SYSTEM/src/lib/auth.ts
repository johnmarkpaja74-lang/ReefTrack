...
async function apiPost<T>(path: string, body: Record<string, unknown>) {
  ...
  const data = await response.json().catch(() => ({ ok: false, message: "The ReefTrack server returned an invalid response." })) as ApiResult<T>
...
export function getSession() {
  return read<(Pick<ReefTrackUser, "id" | "name" | "email" | "role" | "settings"> & { accountId?: string }) | null>(STORAGE_KEYS.session, null)
}
...
export async function authenticate(input: { email: string; password: string; role: Role; remember: boolean }) {
  ...
  if (!user) {
    recordAttempt(users.find((item) => normalizeEmail(item.email) === email), email, input.role, false)
    return { ok: false as const, message: "The email or password is incorrect." }
  }
